...
def load_model(weights, img_size, device, model_type="reliability"):
...
def main():
    ...
    parser.add_argument("--model", default=None, choices=("reliability", "availability_reliability"))
        ...
        runs = (
            {
                ...
                or ("E5 ACRF + Dropout 0.15" if args.model == "availability_reliability" else "Reliability Fusion"),
                "weights": args.weights,
                "model": args.model or "reliability",
            },
        )
    else:
        runs = tuple({**run, "model": "reliability"} for run in RUNS)

    summary_rows = []
    for run in runs:
        print(f"Analyzing {run['method']}")
        model = load_model(run["weights"], args.img_size, device, model_type=run.get("model", "reliability"))
        for mode in MODES:
