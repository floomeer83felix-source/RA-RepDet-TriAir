...
def main() -> int:
    ...
    rationale_path = out_root / "split_build/v40_assignment_rationale.json"
    rationale = json.loads(rationale_path.read_text(encoding="utf-8")) if rationale_path.exists() else {}
    reported_val_gt = int(rationale.get("validation_gt_box_count", -1))
    frozen_val_gt = int(rationale.get("frozen_v39_validation_gt_box_count", -1))
    reported_gt_abs_diff = int(rationale.get("validation_gt_box_abs_diff", -1))
    independent_gt_abs_diff = abs(val_gt - frozen_val_gt) if frozen_val_gt >= 0 else -1
    assignment_accounting_pass = (
        reported_val_gt == val_gt
        and reported_gt_abs_diff == independent_gt_abs_diff
        and frozen_val_gt >= 0
    )
    if not assignment_accounting_pass:
        status = "V40_V2_ASSIGNMENT_ACCOUNTING_FAILED"
    elif all_zero and count_checks:
        status = "V40_V2_READY_FOR_FROZEN_RERUN"
    else:
        status = "V40_V2_SPLIT_AUDIT_FAILED"

    accounting_metrics = [
        {"metric": "reported_validation_gt_boxes", "value": reported_val_gt, "expected": val_gt, "status": "PASS" if reported_val_gt == val_gt else "FAIL"},
        {"metric": "independent_assignment_validation_gt_boxes", "value": val_gt, "expected": val_gt, "status": "PASS"},
        {"metric": "frozen_v39_validation_gt_boxes", "value": frozen_val_gt, "expected": "record", "status": "PASS" if frozen_val_gt >= 0 else "FAIL"},
        {"metric": "reported_validation_gt_abs_diff", "value": reported_gt_abs_diff, "expected": independent_gt_abs_diff, "status": "PASS" if reported_gt_abs_diff == independent_gt_abs_diff else "FAIL"},
        {"metric": "independent_validation_gt_abs_diff", "value": independent_gt_abs_diff, "expected": independent_gt_abs_diff, "status": "PASS" if independent_gt_abs_diff >= 0 else "FAIL"},
        {"metric": "assignment_accounting_pass", "value": assignment_accounting_pass, "expected": "True", "status": "PASS" if assignment_accounting_pass else "FAIL"},
    ]
    write_csv(out_root / "audits/v40_assignment_gt_accounting_audit.csv", accounting_metrics, ["metric", "value", "expected", "status"])
    ...
    payload = {
        ...
        "reported_validation_gt_box_count": reported_val_gt,
        "frozen_v39_validation_gt_box_count": frozen_val_gt,
        "reported_validation_gt_abs_diff": reported_gt_abs_diff,
        "independent_validation_gt_abs_diff": independent_gt_abs_diff,
        "assignment_accounting_pass": assignment_accounting_pass,
    ...
    report_lines = [
        ...
        f"- Reported validation GT boxes: {reported_val_gt}",
        f"- Frozen V39 validation GT boxes: {frozen_val_gt}",
        f"- Reported validation-GT absolute difference: {reported_gt_abs_diff}",
        f"- Independent validation-GT absolute difference: {independent_gt_abs_diff}",
        f"- Assignment accounting pass: {assignment_accounting_pass}",
    ...
    final_payload = {
        **payload,
        "output_paths": {
            "train_manifest": "reproducibility/v40_expanded_adjacency_component_split_v2/manifests/v40_expanded_adjacency_component_disjoint_train.txt",
            "validation_manifest": "reproducibility/v40_expanded_adjacency_component_split_v2/manifests/v40_expanded_adjacency_component_disjoint_val.txt",
            "audit_report": "reproducibility/v40_expanded_adjacency_component_split_v2/audits/v40_split_audit_report.md",
            "rerun_handoff": "reproducibility/v40_expanded_adjacency_component_split_v2/reports/V40_RERUN_HANDOFF.md",
        },
    }
    write_json(out_root / "reports/V40_V2_EXPANDED_ADJACENCY_SPLIT_STATUS.json", final_payload)
    status_lines = [
        "# V40 v2 Expanded-Adjacency Split Status",
        "",
        f"- Generated: {now}",
        f"- Input commit: `{commit}`",
        "- Output commit: `PENDING_FINAL_COMMIT_RECORDED_IN_GIT_HISTORY_AND_FINAL_RESPONSE`",
    ...
    (out_root / "reports/V40_V2_EXPANDED_ADJACENCY_SPLIT_STATUS.md").write_text(
        "\n".join(status_lines) + "\n", encoding="utf-8"
    )
    ...
    return 0 if status == "V40_V2_READY_FOR_FROZEN_RERUN" else 2
