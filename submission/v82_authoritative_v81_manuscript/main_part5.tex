...
The locked holdout strengthens the evidence boundary: AP50 improves in all three seed pairs after checkpoint lock, but F1 and AP75 remain mixed. The result is not uniformly positive across every localization criterion and should not be described as an independent test. The checkpoint-backed V81 replication closes the principal basic-control gap with complete checkpoint and evaluator identity. Thermal is the strongest individual stream across AP, AP50, AP75, AR1, AR10, and AR100. Under compatible standardized evaluator definitions, both matched early fusion and reliability-aware fusion exceed thermal-only in all three paired seeds for AP, AP50, and AP75. Event-only performance remains substantially lower, suggesting that the event representation contributes mainly in combination with the appearance-based streams under the present preprocessing and detector contract. The earlier author-supplied single-modality table is excluded from primary claims because its checkpoint identity package is unavailable and its values differ materially from V81.
...
The fresh checkpoint-backed single-modality replication further shows that thermal is the strongest standalone stream, with $0.5196\pm0.0196$ standardized COCO AP and $0.6473\pm0.0132$ AR100. The full reliability-aware model exceeds thermal-only by $0.1960\pm0.0329$ AP, $0.1215\pm0.0154$ AP50, and $0.2952\pm0.0349$ AP75 across paired seeds, with every difference positive. This closes the basic modality-contribution control with auditable checkpoint identities and supports a multimodal benefit under the frozen TriAir development protocol.

The cross-dataset study further shows that frozen TriAir models fail under a naive unregistered MM-UAV adapter, whereas supervised target-domain alignment recovers useful detection. TriAir initialization and reliability-aware fusion then provide consistent additional AP gains across three corrected seeds. The combined evidence is a reproducible in-domain and supervised-transfer result, not calibrated physical sensor reliability, statistical significance, or generalization without target-domain labels.
