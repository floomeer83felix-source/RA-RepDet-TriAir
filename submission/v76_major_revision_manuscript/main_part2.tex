...
The local TriAir representation contains 10,489 five-channel samples and 30,634 valid vehicle boxes. Among the samples, 9,751 have label text files, 738 have no label text file, and one label text file is empty. The data pipeline treats missing and empty label files as empty-target observations rather than removing them. The sample count and RGB--thermal--event sensing configuration align with the 10,489-frame dataset introduced by Iaboni and Abichandani \cite{iaboni2026trimodal}; because the local label inventory differs from the published annotation count, exact provider version and conversion provenance remain explicitly tracked rather than inferred. Readers should obtain data from the original provider and comply with the applicable terms.
...
The TriAir development program uses the same component-disjoint manifests, 50 epochs, $640\times640$ inputs, batch size 4, learning rate $10^{-4}$, AdamW, no mosaic/mixup/crop/flip/color-jitter augmentation, and fixed seeds 0, 1, and 2. Within each run, the checkpoint with the highest project-local validation AP50 is retained. The standardized COCO evaluator then uses score threshold 0.001, NMS threshold 0.6, and at most 100 detections per image. COCO AP denotes AP@[0.50:0.95]. These are development-validation results because the validation partition participates in checkpoint retention.
