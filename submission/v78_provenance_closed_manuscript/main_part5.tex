...
The locked holdout strengthens the evidence boundary: AP50 improves in all three seed pairs after checkpoint lock, but F1 and AP75 remain mixed. The result is not uniformly positive across every localization criterion and should not be described as an independent test. The completed trained single-modality study closes the principal basic-control gap. Thermal is the strongest individual stream, but both matched early fusion and reliability-aware fusion exceed thermal-only in all three paired seeds for AP50 and AP75. Event-only performance is substantially lower, suggesting that the event representation contributes mainly in combination with the appearance-based streams under the present preprocessing and detector contract.
...
The trained single-modality controls further show that thermal is the strongest standalone stream, but the full reliability-aware model exceeds thermal-only by $0.1037\pm0.0094$ AP50 and $0.2465\pm0.0253$ AP75 across paired seeds. This closes the basic modality-contribution control and supports a genuinely multimodal benefit under the frozen TriAir protocol.
