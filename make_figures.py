...
ax.text(0.2,0.35,'Guard partition is excluded from selection and evaluated once only after all six primary checkpoints are fixed.',fontsize=8.8)
fig.tight_layout()
fig.savefig(OUT/'fig2_protocol.pdf', bbox_inches='tight')
plt.close(fig)

# Fig 3: three-seed standardized COCO comparisons
metrics = ['AP', 'AP50', 'AP75']
early = np.array([0.680255, 0.937245, 0.809032])
rel = np.array([0.715606, 0.953412, 0.872876])
early_sd = np.array([0.022050, 0.004660, 0.025091])
rel_sd = np.array([0.017212, 0.001659, 0.019187])
...
ax.set_ylim(.62,.99)
ax.set_ylabel('Three-seed mean metric')
...
methods = ['Scratch\nEqual', 'TriAir Init\nEqual', 'TriAir Init\nReliability']
means = np.array([0.2210, 0.2340, 0.2503])
stds = np.array([0.0030, 0.0020, 0.0025])
...
for i, value in enumerate(means):
    ax.text(i, value + stds[i] + 0.0012, f'{value:.4f}', ha='center', va='bottom', fontsize=9)
fig.tight_layout()
